...
async function compositeShadowPerspective(
  ...
  const SHADOW_Y_OFFSET = 25;   // tweak this (10–40 px) depending on model
  dstQuadPixel.forEach(p => {
    p.y += SHADOW_Y_OFFSET;
  });
...
function place3DModelInCanvas() {
  ...
  loader.load(lastModelURL, obj => {
    ...
    obj.rotation.x -= 0.01;   // small forward/back tilt
    obj.rotation.z += 0.02;   // small left/right tilt
    ...
    // this help to move shadow up and down
    const SHADOW_PLANE_Y = -0.25;
