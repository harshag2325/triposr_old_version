//update - correct in chair position not side ways
...
function initViewer3D() {
  ...
  viewerCamera = new THREE.PerspectiveCamera(35, width / height, 0.1, 1000);
  viewerCamera.position.set(0, 0.25, 3);
...
function load3DModel(modelUrl, textureUrl) {
  ...
  loader.load(
    ...
    function (obj) {
      ...
      const size_1 = new THREE.Vector3();
      bbox.getSize(size_1);

      // 4) shift down so the lowest point is at y = 0 (ground plane)
      const minY = bbox.min.y !== undefined ? bbox.min.y : 0;
      obj.position.y -= minY;
      obj.updateMatrixWorld(true);
      const geomList = [];
      obj.traverse(child => {
        ...
        // 2) find 40 lowest vertices
        geomList.sort((a, b) => a.y - b.y);
        const sample = geomList.slice(0, 40);

      // 3) compute best-fit plane normal from lowest points
        let mean = new THREE.Vector3();
        sample.forEach(v => mean.add(v));
        mean.divideScalar(sample.length);

        let cov = new THREE.Matrix3();
        cov.set(0,0,0, 0,0,0, 0,0,0);

        sample.forEach(v => {
        const p = new THREE.Vector3().subVectors(v, mean);
        cov.elements[0] += p.x * p.x;
        cov.elements[1] += p.x * p.y;
        cov.elements[2] += p.x * p.z;

        cov.elements[3] += p.y * p.x;
        cov.elements[4] += p.y * p.y;
        cov.elements[5] += p.y * p.z;

        cov.elements[6] += p.z * p.x;
        cov.elements[7] += p.z * p.y;
        cov.elements[8] += p.z * p.z;
      });

      // 4) smallest eigenvector = floor normal
      // (approx solve using direct method)
      const normal = new THREE.Vector3();
      if (cov.elements[0] < cov.elements[4]) normal.set(1,0,0);
      else normal.set(0,1,0);

      // cross-product refine direction
      normal.crossVectors(
        new THREE.Vector3(cov.elements[1], cov.elements[4], cov.elements[7]),
        new THREE.Vector3(cov.elements[2], cov.elements[5], cov.elements[8])
      ).normalize();

      // 5) compute quaternion to rotate it flat
...
function place3DModelInCanvas() {
  ...
  loader.load(
    ...
    (obj) => {
      ...
      obj.position.y -= minY;
      obj.updateMatrixWorld(true);
      // === AUTO-LEVEL MODEL (remove tiny tilt) ===

      // 1) collect all vertices in world space
      const geomList = [];
      obj.traverse(child => {
        if (child.isMesh && child.geometry) {
            const g = child.geometry;
