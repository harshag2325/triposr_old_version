...
async function captureThenBlend() {
  ...
  try {
    ...
    // 3) composite with correct foot-anchored homography (no forced override)
    ...
    // 5) compute placement relative to Fabric wrapper
    const obj = window.fabric3D;

    const W = Math.round(obj.scaleX * overlay.offsetWidth);
    const H = Math.round(obj.scaleY * overlay.offsetHeight);
...
async function generateShadowNow() {
  ...
  if (choice === "auto") {
    // use sunLight world direction (from light position to origin)
    if (!window.sunLight) {
      alert("Sun light not ready; using default");
      worldDir = new THREE.Vector3(1, 0, 0);
    } else {
      // direction from light towards origin (shadow direction is opposite)
      worldDir = window.sunLight.position.clone().normalize().negate();
    }
  } else {
    // define local direction vectors in model space
    let local;
    switch (choice) {
      case "left":  local = new THREE.Vector3(-1, 0, 0); break;
      case "right": local = new THREE.Vector3(1, 0, 0); break;
      case "front": local = new THREE.Vector3(0, 0, -1); break; // toward camera
      case "back":  local = new THREE.Vector3(0, 0, 1); break;
      default: local = new THREE.Vector3(1, 0, 0);
    }
    // rotate by model's world quaternion to get world direction
    const q = new THREE.Quaternion();
    window.model3D.getWorldQuaternion(q);
    worldDir = local.applyQuaternion(q).normalize();
    // shadow is cast in opposite direction from light; invert
    worldDir = worldDir.clone().negate();
  }

  // compute overlay pixel direction
  const shadowDir = computeOverlayShadowDirFromWorldDir(worldDir) || { sx: 1, sy: 0 };

  // Now capture and composite using the computed shadowDir
  try {
    const overlay = document.getElementById("threeCanvasInFabric");
    const overlayW = overlay.clientWidth;
    const overlayH = overlay.clientHeight;
    ...
    // place blended final image into canvas
    const obj = window.fabric3D;
    const W = Math.round(obj.scaleX * overlay.offsetWidth);
    const H = Math.round(obj.scaleY * overlay.offsetHeight);
    const X = Math.round(obj.left);
    ...
    alert("Generate shadow failed: " + (err.message || err));
  }
}
...
function place3DModelInCanvas() {
  ...
  // create a small Generate Shadow button inside overlay controls area
  const genBtn = document.createElement("button");
  genBtn.textContent = "Generate Shadow";
  genBtn.style.position = "absolute";
  genBtn.style.right = "8px";
  genBtn.style.bottom = "8px";
  genBtn.style.zIndex = "9999";
  genBtn.onclick = () => generateShadowNow();
