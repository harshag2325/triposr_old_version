/* Fixed editor.js — shadow blending fixes
   - uses fabric overlay scaled width/height/left/top for blend coords
   - computes overlay pixel shadow direction from 3D world light when possible
   - sends base64 string as image_data to /upload_3d_view (matches your backend)
*/
...
// safer flexible fetch (returns parsed JSON or {raw: text})
...
// Homography utilities (computeHomography, solveLinear, invert3x3)
...
function getModelFootPointsInOverlay() {
  try {
    ...
    if (!obj || !camera || !renderer || !overlay) return [];

    const rd = renderer.domElement.getBoundingClientRect();
    const ov = overlay.getBoundingClientRect();

    // offset from renderer canvas to overlay in page coordinates
    const offsetX = ov.left - rd.left;
    const offsetY = ov.top - rd.top;

    // use renderer drawing buffer size (CSS pixels used earlier by setSize)
    ...
    for (const p of feet) {
      ...
      // map NDC -> renderer pixel space
      const rx = ((ndc.x + 1) / 2) * W;
      const ry = ((-ndc.y + 1) / 2) * H;

      // subtract offset to get overlay-local pixel coords
      result.push({
        x: rx - offsetX,
        y: ry - offsetY
      });
...
async function compositeShadowPerspective(
  ...
  opts = Object.assign({
    ...
    blurPx: 18,
    shadowOpacity: 0.55,
    shadowDir: null // optional {sx, sy} in overlay pixel space
  }, opts);
  ...
  // fabric box (use fabric3D scaled sizes and left/top from fabric object)
  const obj = window.fabric3D;
  const left = obj.left;
  const top = obj.top;
  ...
  // compute box normalized positions in overlay pixel coords
  const boxNorm = pt => ({
  ...
  // compute shadow direction sx,sy (in overlay pixel coords)
  let sx = 0, sy = 0;
  if (opts.shadowDir && typeof opts.shadowDir.sx === 'number') {
    ...
    // direction away from sun -> shadow
  ...
  if (feet.length >= 2) {
    ...
    // use normalized step in direction then scale
    const sgnX = sx === 0 ? 1 : sx / Math.abs(sx);
    const sgnY = sy === 0 ? 1 : sy / Math.abs(sy);

    dstBottomLeft = {
      x: leftFoot.x + sgnX * extend,
      y: leftFoot.y + sgnY * extend * squash
    };

    dstBottomRight = {
      x: rightFoot.x + sgnX * extend,
      y: rightFoot.y + sgnY * extend * squash
    };
    ...
    dstBottomLeft = {
      x: norm_bl.x + sgnX * extend,
      y: norm_bl.y + sgnY * extend * squash
    };

    dstBottomRight = {
      x: norm_br.x + sgnX * extend,
      y: norm_br.y + sgnY * extend * squash
    };
  }
  ...
  // convert to normalized destination coordinates (0..1)
...
async function captureThenBlend() {
  ...
  // overlay pixel size used throughout for homography (CSS pixels)
  ...
  try {
    ...
    // try compute shadowDir from 3D world/sun
    let shadowDir = null;
    if (window.sunLight && window.model3D && window.threeCamera && window.threeRenderer) {
      // direction from model toward ground along light vector
      const sunPos = window.sunLight.position.clone();
      const target = window.sunLight.target ? window.sunLight.target.position.clone() : new THREE.Vector3(0,0,0);
      const worldDir = new THREE.Vector3().subVectors(target, sunPos); // light direction (towards target)
      shadowDir = computeOverlayShadowDirFromWorldDir(worldDir);
    }
    ...
    // send base64 string (your backend expects image_data string)
    const f1 = new FormData();
    f1.append("image_data", combinedDataURL);
    ...
    // Use fabric3D's actual scaled pixel dimensions & position
    const obj = window.fabric3D;
    ...
    alert("3D blend failed → " + (err && err.message ? err.message : err));
  }
}
window.captureThenBlend = captureThenBlend;

// computeOverlayShadowDirFromWorldDir
...
// generateShadowNow — main entrypoint for the user pressing "Generate Shadow"
async function generateShadowNow() {
  ...
  try {
    ...
    // compute shadowDir from 3D world if possible
    let shadowDir = null;
    if (window.sunLight && window.model3D && window.threeCamera && window.threeRenderer) {
      const sunPos = window.sunLight.position.clone();
      const target = window.sunLight.target ? window.sunLight.target.position.clone() : new THREE.Vector3(0,0,0);
      const worldDir = new THREE.Vector3().subVectors(target, sunPos);
      shadowDir = computeOverlayShadowDirFromWorldDir(worldDir);
    }
    ...
    alert("Generate shadow failed → " + (err && err.message ? err.message : err));
  }
}
window.generateShadowNow = generateShadowNow;

// place3DModelInCanvas (mostly same as earlier)
  ...
  loader.load(lastModelURL, obj => {
    ...
    obj.traverse(c => {
      ...
      for (let i = 0; i < pos.count; i++) {
        let y = pos.getY(i);
        if (y < minRealY) minRealY = y;
      }
    });

    const SHADOW_PLANE_Y = -0.6;
    obj.position.y = SHADOW_PLANE_Y - minRealY;
